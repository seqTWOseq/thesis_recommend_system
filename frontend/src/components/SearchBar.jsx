...
function SearchBar({ onSearch, loading = false }) {
  ...
  const innerBarClass =
    "w-full max-w-3xl rounded-full bg-[#FFFFFF] shadow-sm px-2 py-2 flex items-center gap-2 transition-shadow " +
    (loading
      ? "focus-within:ring-0"
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full flex justify-center">
      {loading ? (
        <div className="search-bar-rainbow-ring w-full max-w-3xl shadow-sm rounded-full">
          <div className={`search-bar-rainbow-inner ${innerBarClass}`}>{fields}</div>
        </div>
      ) : (
